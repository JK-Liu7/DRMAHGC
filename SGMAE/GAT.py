...
class GAT(nn.Module):
    def __init__(self, args, in_dim, out_dim, num_hidden, num_layers, num_out_heads, encoding=False):
        ...
        if num_layers == 1:
            self.gat_layers.append(GATConv(
                in_dim, out_dim, self.num_out_heads,
                self.in_drop, self.attn_drop, self.negative_slope, last_residual))
        else:
            # input projection (no residual)
            self.gat_layers.append(GATConv(
                in_dim, self.num_hidden, self.num_heads,
                self.in_drop, self.attn_drop, self.negative_slope, self.residual, activation=nn.PReLU()))
            # hidden layers
            for l in range(1, num_layers - 1):
                # due to multi-head, the in_dim = num_hidden * num_heads
                self.gat_layers.append(GATConv(
                    self.num_hidden * self.num_heads, self.num_hidden, self.num_heads,
                    self.in_drop, self.attn_drop, self.negative_slope, self.residual, activation=nn.PReLU()))
            # output projection
            self.gat_layers.append(GATConv(
                self.num_hidden * self.num_heads, out_dim, self.num_out_heads,
